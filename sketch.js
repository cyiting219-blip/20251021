...
function drawCenteredText() {
    ...
    textSize(48); // 設定文字大小為 48
    textAlign(CENTER, CENTER); // 設定文字對齊方式
    text("414730373陳奕廷", width / 2, height / 2); // 在畫布正中間顯示文字
}

function mousePressed() {
    // 檢查是否點擊了「選單項目 1」
    if (mouseX < menuWidth && mouseY > 10 && mouseY < 30) {
        iframe.attribute('src', 'https://cyiting219-blip.github.io/20251014-/');
        iframeVisible = true;
        iframe.show();
    }
    // 檢查是否點擊了「選單項目 2」
    else if (mouseX < menuWidth && mouseY > 40 && mouseY < 60) {
        iframe.attribute('src', 'https://hackmd.io/@ITh9xKDuSoGWeaOQFpb1-w/HygtxtOJnel');
        iframeVisible = true;
        iframe.show();
    }
    // 檢查是否點擊了「選單項目 3」
    else if (mouseX < menuWidth && mouseY > 70 && mouseY < 90) {
        iframeVisible = false;
